(() => {
  ...
  // node_modules/.pnpm/gsap@3.12.7/node_modules/gsap/utils/paths.js
  ...
  // node_modules/.pnpm/gsap@3.12.7/node_modules/gsap/CustomEase.js
  var gsap2;
  var _coreInitted2;
  ...
  function createHoverAnimation(card, initialStates, timelineActions) {
    ...
    Object.entries(initialStates).forEach(([selector3, props]) => {
      const element = card.querySelector(selector3);
      if (element)
        gsapWithCSS.set(element, props);
    });
    timelineActions.forEach((action) => {
      const elements = action.selectors.map((selector3) => card.querySelector(selector3)).filter(Boolean);
      timeline2.to(elements, action.props, action.position || "<");
    });
  ...
  function initializeResourceAnimations(items = document) {
    Object.entries(resourceAnimations).forEach(([className, config3]) => {
      let cards;
      if (items === document) {
        ...
        items.forEach((item) => {
          if (item.element) {
            if (item.element.classList && item.element.classList.contains("resource_layout") && item.element.classList.contains(className)) {
      ...
      cards.forEach((card) => {
        if (!card.dataset.animationInitialized && !isElementHidden(card)) {
          createHoverAnimation(card, config3.initialStates, config3.timelineActions);
