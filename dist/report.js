  ...
  window.Webflow.push(() => {
    const getUrlParameter = function getUrlParameter2(sParam) {
      ...
      for (i = 0; i < sURLVariables.length; i++) {
        ...
        if (sParameterName[0] === sParam) {
          return typeof sParameterName[1] === void 0 ? true : decodeURIComponent(sParameterName[1]);
        }
      }
      return "";
    };
    ...
    document.querySelectorAll('[fs-modal-element="close-1"]').forEach((link) => {
      link.addEventListener("click", () => {
        document.querySelector('[fs-modal-element="modal-1"] .fs_modal_video .fs_modal_video-embed').remove();
      });
    });
    ...
    function animateNumber(element, numberFloat, numberString, duration2, decimal) {
      let start = null;
      let someValue = 0;
      function step(timestamp) {
        ...
        someValue = numberFloat * progress / duration2;
        if (someValue > numberFloat)
          someValue = numberFloat;
        element.textContent = numberString[0] + (Math.round((someValue + Number.EPSILON) * 1 * decimal) / (1 * decimal)).toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") + numberString[1];
