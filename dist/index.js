(() => {
  ...
  window.Webflow.push(() => {
    ...
    function setupTabs() {
      ...
      gsapWithCSS.set(".autotab_image-wrapper:nth-child(1)", { zIndex: 5, backgroundColor: "white" });
      ...
      tabs.forEach((item, index) => {
        ...
        if (index === 0) {
          gsapWithCSS.set(body, { height: "auto" });
          gsapWithCSS.set(line, { opacity: 1 });
          gsapWithCSS.set(lineInner, { height: "0%" });
        } else {
          gsapWithCSS.set(body, { height: 0 });
          gsapWithCSS.set(line, { opacity: 0 });
          gsapWithCSS.set(lineInner, { height: "0%" });
        }
    ...
    function animateTab(tab, isActive, isManualClick = false) {
      ...
      if (isActive) {
        lineAnimation = gsapWithCSS.to(lineInner, {
          height: "100%",
          duration: isManualClick ? AUTO_PLAY_DELAY / 1e3 : 6,
          ease: "none",
          onComplete: () => {
            if (!isHovering) {
              nextTab();
    ...
    function initTabComponent() {
      setupTabs();
      scrollTrigger = ScrollTrigger2.create({
    ...
    mm.add("(min-width: 992px)", () => {
      if (!isDesktopView) {
        ...
        resetTabsForDesktop();
        initTabComponent();
        if (ScrollTrigger2.isInViewport(tabComponent)) {
          startAnimation();
        }
      }
      return () => {
    ...
    function resetTabsForMobile() {
      ...
      images.forEach((image) => {
        gsapWithCSS.set(image, { opacity: 1, visibility: "visible" });
      });
    }
